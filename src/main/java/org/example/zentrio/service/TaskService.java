...
public interface TaskService {
    Task createTaskByBoardIdAndGanttBarId(TaskRequest taskRequest, UUID boardId, UUID ganttBarId);

    Task getTaskById(UUID taskId);

    ApiResponse<List<Task>> getAllTasksByBoardIdAndGanttBarId(UUID boardId, UUID ganttBarId, Integer page, Integer size);

    Task updateTaskByIdWithBoardIdAndGanttBarId(TaskRequest taskRequest, UUID taskId, UUID boardId, UUID ganttBarId);
